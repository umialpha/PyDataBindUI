...
float  SRGBToLinearApproximate(  float3  inColor )
{
...
float  LinearToSRGBApproximate(  float3  inColor )
{
	return sqrt( inColor );
...
texture SceneDepthTexture : DepthBuffer;
sampler SceneDepthTextureSampler = sampler_state
{
...
float CalcSceneDepth( float2 ScreenUV )
{
	...
	return LinearEyeDepth(Texture2DSampleLevel(SceneDepthTexture, SceneDepthTextureSampler, float2(ScreenUV.x, 1 - ScreenUV.y), 0).r);
}
...
half3 GetNormal( MaterialPixelParameters Parameters )
{
	return  float4 (float3(0.00000000,0.00000000,1.00000000),0).rgb;
}

half3 GetEmissiveRaw( MaterialPixelParameters Parameters )
{
	float2  Local0 = (Parameters.TexCoords[0].xy + VectorUniformExpressions[0].rg);
	float2  Local1 = (VectorUniformExpressions[1].rg * Local0);
	float4  Local2 = ProcessMaterialColorTextureLookup(Texture2DSample(Tex0,Tex0Sampler,Local1));
	float3  Local3 = (VectorUniformExpressions[2].rgb * Local2.rgb);
	return Local3;
}
...
{
	float2  Local4 = (Parameters.TexCoords[0].xy + VectorUniformExpressions[0].rg);
	float2  Local5 = (VectorUniformExpressions[1].rg * Local4);
	float4  Local6 = ProcessMaterialColorTextureLookup(Texture2DSample(Tex0,Tex0Sampler,Local5));
	float  Local7 = (ScalarUniformExpressions.x * Local6.a);
	return Local7;
}
...
{
	return float2(float3(1.00000000,0.00000000,0.00000000).r,ScalarUniformExpressions.y);;
}
...
void SetUV( inout VertexFactoryOutput Interpolants, int UVIndex, float2 InValue )
{
	Interpolants.TexCoords[UVIndex] = InValue;
}
...
VertexFactoryOutput VF_GetVertexFactoryOutput( VertexInput Input, VertexFactoryLocals Locals, MaterialVertexParameters Params )
{
	...
	GetMaterialCustomizedUVs( Params, CustomizedUVs );
...

{
	...
	float3 DiffuseColor = GetDiffuseColor(Parameters);
	float3 SpecularColor = GetSpecularColor(Parameters);
	float DiffusePower = GetDiffusePower(Parameters);
	float SpecularPower = GetSpecularPower(Parameters);

	half3 viewDirection = Parameters.CameraVector;
	half3 normalDirection = Parameters.WorldNormal;
	half3 lightDirection = normalize( -LightDir .xyz);
	half3 lightColor =  DirLightAttr[1].rgb .rgb;
	half3 halfDirection = normalize(viewDirection + lightDirection);



	half attenuation = 1.0;
	half3 attenColor = attenuation * lightColor.xyz;

	half NdotL = max(0, dot(normalDirection, lightDirection));
	half3 directSpecular = (floor(attenuation) * lightColor.xyz) * pow(max(0, dot(halfDirection, normalDirection)), SpecularPower)*SpecularColor;

	half3 directDiffuse = pow(max(0.0, NdotL), half3(DiffusePower, DiffusePower, DiffusePower)) * attenColor;
	half3 indirectDiffuse = float3(0, 0, 0);
	indirectDiffuse += Ambient.rgb;
